...
inline void to_json(nlohmann::json& j, const glm::mat4& m) {
    ...
    for (int i = 0; i < 16; ++i) j.push_back(glm::value_ptr(m)[i]);
}

inline void from_json(const nlohmann::json& j, glm::mat4& m) {
    for (int i = 0; i < 16; ++i) glm::value_ptr(m)[i] = j.at(i).get<float>();
}
...
void sendEventClientToServer(nlohmann::json event_serialized) { clientSideEventSendQueue.push_back(event_serialized); }

void sendEventServerToClient(nlohmann::json event_serialized) { serverSideEventSendQueue.push_back(event_serialized); }

std::vector<glm::mat4> clientSidePredictCurrentModelMatrices(const std::vector<glm::mat4>& previousPrediction,
...
void serverSceneStateUpdateEventHandler(const ServerSceneStateUpdateEvent&& event_, SceneState& sceneState) {
    ...
    sceneState.modelMatrices = clientSidePredictCurrentModelMatrices(sceneState.modelMatrices, modelMatrices,
                                                                     sceneState.previousPredictionTime, server_time);
}
...
void dispatchEventFromClient(nlohmann::json serializedEvent) { return; }
...
UniformBufferObject createUniformBufferObject(const SceneState& sceneState, float aspectRatio) {
    ...
    auto fovy = 1.0;
    // The aspect ratio is taken from the live framebuffer rather than the camera so the
    // scene is not stretched when the window is resized; instead a wider/narrower window
    // reveals more/less of the scene around the camera's focus (issue #9).
    auto aspect = static_cast<double>(aspectRatio);
    auto znear = 0.1;
    auto zfar = 100.0;
...
AABB findExtremaVertices(std::vector<VertexPosNorCol>& verts) {
    const auto& minXIt = std::min_element(
        verts.begin(), verts.end(),
        [](const VertexPosNorCol& rhs, const VertexPosNorCol& lhs) { return (rhs.pos.x < lhs.pos.x); });
    const auto& minYIt = std::min_element(
        verts.begin(), verts.end(),
        [](const VertexPosNorCol& rhs, const VertexPosNorCol& lhs) { return (rhs.pos.y < lhs.pos.y); });
    const auto& minZIt = std::min_element(
        verts.begin(), verts.end(),
        [](const VertexPosNorCol& rhs, const VertexPosNorCol& lhs) { return (rhs.pos.z < lhs.pos.z); });
    const auto& maxXIt = std::max_element(
        verts.begin(), verts.end(),
        [](const VertexPosNorCol& rhs, const VertexPosNorCol& lhs) { return (rhs.pos.x < lhs.pos.x); });
    const auto& maxYIt = std::max_element(
        verts.begin(), verts.end(),
        [](const VertexPosNorCol& rhs, const VertexPosNorCol& lhs) { return (rhs.pos.y < lhs.pos.y); });
    const auto& maxZIt = std::max_element(
        verts.begin(), verts.end(),
        [](const VertexPosNorCol& rhs, const VertexPosNorCol& lhs) { return (rhs.pos.z < lhs.pos.z); });
    ...
    const AABB& aabb{{minX, minY, minZ, 1}, {maxX, maxY, maxZ, 1}};
...
void castRayForMouseHoverOnNode(const ENG::Node& node, const SceneState& sceneState, const glm::vec3& rayOrigin,
...
void castRayForMouseHover(const WindowUserData& windowUserData, const SceneState& sceneState,
    ...
    // "normalized device coordinates (NDC)" the coordinates centered around 0,0 on the screen ranging from -1,1 to 1,-1
    // top-left to bottom-right
    const auto& mouseXNDC = (2.f * windowUserData.cursorXScreenCoords) / windowUserData.windowWidthScreenCoords - 1.0f;
    const auto& mouseYNDC =
        1.f - (2.f * windowUserData.cursorYScreenCoords) / (windowUserData.windowHeightScreenCoords);

    // a vector of 4x4 model matrices representing the rotation, translation, and scale of each model in the scene,
    // indexed by nodeId
...
void mesh_bind_event_handler(VkRenderer& renderer, SceneState& sceneState, VkAdapter& adapter,
                             BindHostMeshDataEvent&& bindEvent) {
    ...
    while (!adapter.graphicsEventQueue.empty()) {
        ...
        if (std::holds_alternative<BindHostMeshDataEvent>(graphicsEvent)) {
            mesh_bind_event_handler(renderer, sceneState, adapter,
                                    std::move(std::get<BindHostMeshDataEvent>(graphicsEvent)));
