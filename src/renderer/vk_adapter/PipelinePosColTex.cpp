...
Pipeline_PosColTex::Pipeline_PosColTex(const VkDevice device, const VkRenderPass& renderPass,
                                       const ShaderFactory& shader_fac,
...
Pipeline_PosBB::Pipeline_PosBB(const VkDevice device, const VkRenderPass& renderPass, const ShaderFactory& shader_fac,
                               std::vector<VkGraphicsPipelineCreateInfo>& pipelineCreateInfos)
    : Pipeline(device) {
    Initialize(renderPass, shader_fac, pipelineCreateInfos);
}
...
void Pipeline_Goldberg::createDescriptorSetLayout(const VkDevice device) {
    ...
    std::array<VkDescriptorSetLayoutBinding, 4> bindings = {uboLayoutBinding, modelMatrixBinding,
                                                            faceColorMatrixBinding, faceIdMapBufferBinding};
    VkDescriptorSetLayoutCreateInfo layoutInfo{};
    layoutInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
    layoutInfo.bindingCount = static_cast<uint32_t>(bindings.size());
    layoutInfo.pBindings = bindings.data();
