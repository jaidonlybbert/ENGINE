...
using GraphicsEvent = std::variant<BindHostMeshDataEvent, CommandRecorderEvent, CommandCompletionEvent>;
...
class VkAdapter : public RenderAdapterI {
   ...
   public:
    ...
    ~VkAdapter() {
        for (auto& drawData : drawDataBuffer) {
            if (drawData.bufferAllocationInfo.has_value()) {
                vmaDestroyBuffer(vmaAllocator, drawData.bufferAllocationInfo.value().vertexBuffers[0],
                                 drawData.bufferAllocationInfo.value().vertexAllocation);
                vmaDestroyBuffer(vmaAllocator, drawData.bufferAllocationInfo.value().indexBuffer,
    ...
    DrawDataAllocationInfo create_draw_data(VertexT&& vertices, std::vector<uint32_t>&& indices) {
        ...
        const auto* vertexData = std::holds_alternative<std::vector<VertexPosColTex>>(vertices)
                                     ? static_cast<void*>(std::get<std::vector<VertexPosColTex>>(vertices).data())
                                 : std::holds_alternative<std::vector<VertexPosNorCol>>(vertices)
                                     ? static_cast<void*>(std::get<std::vector<VertexPosNorCol>>(vertices).data())
                                 : std::holds_alternative<std::vector<VertexPosNorTex>>(vertices)
                                     ? static_cast<void*>(std::get<std::vector<VertexPosNorTex>>(vertices).data())
                                     : nullptr;
        ...
        vmaCreateBuffer(vmaAllocator, &vbInfo, &gpuAlloc, &drawDataInfo.vertexBuffers[0],
                        &drawDataInfo.vertexAllocation, &drawDataInfo.vertexAllocationInfo);
        vmaCreateBuffer(vmaAllocator, &ibInfo, &gpuAlloc, &drawDataInfo.indexBuffer, &drawDataInfo.indexAllocation,
                        &drawDataInfo.indexAllocationInfo);
        ...
        vmaCreateBuffer(vmaAllocator, &stagingInfoVB, &stagingAlloc, &stagingVB, &stagingVBAlloc, nullptr);
        ...
        vmaCreateBuffer(vmaAllocator, &stagingInfoIB, &stagingAlloc, &stagingIB, &stagingIBAlloc, nullptr);
        ...
        graphicsEventQueue.push(CommandRecorderEvent{[this, stagingVB, stagingIB, stagingVBAlloc, stagingIBAlloc,
                                                      drawDataInfo, vertexSize, indexSize](VkCommandBuffer cmdBuffer) {
        ...
        graphicsEventQueue.push(CommandCompletionEvent{[this, stagingVB, stagingIB, stagingVBAlloc, stagingIBAlloc]() {
