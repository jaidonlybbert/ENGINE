...
static VkFormat get_vk_format_from_tinygltf_accessor(const tinygltf::Accessor& acc, size_t& size_bytes) {
    ...
    if (ctype == TINYGLTF_COMPONENT_TYPE_FLOAT) {
        retval = type == TINYGLTF_TYPE_VEC2   ? VkFormat::VK_FORMAT_R32G32_SFLOAT
                 : type == TINYGLTF_TYPE_VEC3 ? VkFormat::VK_FORMAT_R32G32B32_SFLOAT
                 : type == TINYGLTF_TYPE_VEC4 ? VkFormat::VK_FORMAT_R32G32B32A32_SFLOAT
...
void get_vertex_and_index_buffer(const tinygltf::Primitive& primitive, const tinygltf::Model& model,
                                 std::vector<VertexPosNorTex>& vertices, std::vector<uint32_t>& indices) {
    ...
    for (size_t i = 0; i < num_elements; ++i) {
        ...
        std::memcpy(&vertices.at(i).normal, &nor_buff.data[nor_bv.byteOffset + i * nor_size],
                    sizeof(vertices[0].normal));
        std::memcpy(&vertices.at(i).texCoord, &tex_buff.data[tex_bv.byteOffset + i * tex_size],
                    sizeof(vertices[0].texCoord));
    }
