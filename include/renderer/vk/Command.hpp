...
class Command {
   public:
    ...
    void copyBufferToImage(const VkQueue& graphicsQueue, VkBuffer buffer, VkImage image, uint32_t width,
                           uint32_t height);
    void transitionImageLayout(const VkQueue& graphicsQueue, VkImage image, VkFormat format, VkImageLayout oldLayout,
