...
void PipelineFactory::initialize(const VkDevice device, const VkFormat& swapChainImageFormat,
    ...
    eng_pipelines.emplace_back(
        std::make_unique<Pipeline_PosColTex>(device, renderPass, shader_factory, pipelineCreateInfos));
    eng_pipelines.emplace_back(
        std::make_unique<Pipeline_PosNorTex>(device, renderPass, shader_factory, pipelineCreateInfos));
    eng_pipelines.emplace_back(
        std::make_unique<Pipeline_PosBB>(device, renderPass, shader_factory, pipelineCreateInfos));
    eng_pipelines.emplace_back(
        std::make_unique<Pipeline_PosNorCol>(device, renderPass, shader_factory, pipelineCreateInfos));
    eng_pipelines.emplace_back(
        std::make_unique<Pipeline_Goldberg>(device, renderPass, shader_factory, pipelineCreateInfos));
    ...
    const auto success = vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, pipelineCreateInfos.size(), pCreateInfos,
                                                   nullptr, pPipelines);
...
void PipelineFactory::createRenderPass(const VkDevice device, const VkFormat& swapChainImageFormat,
                                       const VkFormat& depthFormat) {
...
const std::vector<VkPipeline>& PipelineFactory::getVkPipelines() const { return graphicsPipelines; }

const std::vector<std::unique_ptr<ENG::Pipeline>>& PipelineFactory::getEngPipelines() const { return eng_pipelines; }

const VkRenderPass& PipelineFactory::getRenderPass() const { return renderPass; }
