...
bool ray_polygon_intersection(const glm::vec3 ray_origin, const glm::vec3 ray_direction,
    ...
    const auto& arbitraryPointOnPlane =
        start + 0.67f * (end1 - start);  // a point 2/3 between first two points on polygon
...
std::optional<glm::vec3> ray_line_intersection(const glm::vec3& ray_origin, const glm::vec3& ray_direction,
                                               const glm::vec3& segment_start, const glm::vec3& segment_end) {
