...
void VkRenderer::cleanupVulkan() {
    ...
    if (enableValidationLayers) {
        ENG::InstanceFactory::DestroyDebugUtilsMessengerEXT(instanceFactory->instance, instanceFactory->debugMessenger,
                                                            nullptr);
    }
...
void VkRenderer::initVulkan() {
    ...
    createDepthResources(device, physicalDevice, swapchain->swapChainExtent, swapchain->depthImage,
...
void VkRenderer::createUniformBuffers() {
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        uniformBuffers.emplace_back(device, physicalDevice, bufferSize, bufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        ...
        if (shaderId == "PosBB" || shaderId == "PosNorCol") {
            descriptorWrites = {
                ...
                createWriteDescriptorSet(descriptorSets.at(i), modelMatrixBufferInfo, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
                                         1),
            };
        } else {
            if (!texturePath.has_value()) {
...
void VkRenderer::createTextureImage(const std::filesystem::path& fpath) {
    ...
    const ENG::Buffer stagingBuffer(device, physicalDevice, 4, imageSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                                    VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
    ...
    createImage(device, physicalDevice, texWidth, texHeight, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL,
                ...
                textureImage, textureImageMem);

    commands->transitionImageLayout(graphicsQueue, textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED,
                                    VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
    commands->copyBufferToImage(graphicsQueue, stagingBuffer.buffer, textureImage, static_cast<uint32_t>(texWidth),
                                static_cast<uint32_t>(texHeight));
    commands->transitionImageLayout(graphicsQueue, textureImage, VK_FORMAT_R8G8B8A8_SRGB,
...
void VkRenderer::createTextureImageView(const std::filesystem::path& fpath) {
    ...
    auto texImageViewInsertionRes = textureImageViews.emplace(
...
void checkedVkMapMemory(VkPhysicalDevice physicalDevice, VkDevice device, VkDeviceMemory bufferMemory,
                        VkDeviceSize dataSize, void** hostData) {
    VkPhysicalDeviceProperties properties;
    vkGetPhysicalDeviceProperties(physicalDevice, &properties);
