...
void VkAdapter::init(size_t sceneSize) { renderer.createModelMatrices(sizeof(glm::mat4) * sceneSize); }
...
void VkAdapter::recordCommandsForSceneGraph2(VkRenderer& renderer, VkCommandBuffer& commandBuffer,
    ...
    for (const auto& node : sceneState.graph.nodes) {
        ...
        vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                                renderer.pipelineFactory.getVkPipelineLayout(shaderId), 0, 1, descriptorSets.data(), 0,
                                nullptr);

        const PushConstants pushConstants{node.nodeId};
        vkCmdPushConstants(commandBuffer, renderer.pipelineFactory.getVkPipelineLayout(shaderId),
