...
void dual(pmp::SurfaceMesh& mesh) {
    ...
    for (auto v : mesh.vertices()) {
        ...
        for (auto f : mesh.faces(v)) vertices.push_back(fvertex[f]);
...
void load_pmp_mesh(ENG::Node& parent, const pmp::SurfaceMesh& mesh, const std::string& mesh_name,
                   const std::string& node_name, const glm::vec4& color, SceneState& sceneState) {
    ...
    sceneState.hostMeshDataBindQueue.push(
        BindHostMeshDataEvent{HostMeshData{std::move(vertices), std::move(indices), "PosNorCol"}, pmpNode.nodeId});
}
